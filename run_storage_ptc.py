"""Implements PTC baselines for storage problem.
...
from storage.problems import StorageProblemBox, StorageProblemEllipsoid
...
def ptc_box(
    ...
    tensors, (y_mean, y_std) = get_tensors(shuffle=shuffle, log_prices=LOG_PRICES)
    results = []
    for seed in tqdm(SEEDS):
        tensors_cv, _ = get_train_calib_split(tensors, seed=seed)
        loaders = get_loaders(tensors_cv, batch_size=BATCH_SIZE)
        prob = StorageProblemBox(T=Y_DIM, y_mean=y_mean, y_std=y_std)

        mlp = MLP(input_dim=INPUT_DIM, y_dim=Y_DIM)
        ...
        for lr, l2reg in itertools.product((1e-4, 1e-3, 1e-2), (0, 1e-4, 1e-3, 1e-2)):
            quantile_mlp, quantile_mlp_result = train_mlp_regressor(
                residual_loaders, pinball_loss_fn, input_dim=INPUT_DIM, y_dim=Y_DIM,
...
def ptc_ellipse(
    ...
    for seed in tqdm(SEEDS):
        ...
        if os.path.exists(ckpt_path):
            ...
            quantile_mlp = MLP(input_dim=INPUT_DIM, y_dim=1, pos=True)
            ...
            for lr, l2reg in itertools.product((1e-4, 1e-3, 1e-2), (0, 1e-4, 1e-3, 1e-2)):
                quantile_mlp, quantile_mlp_result = train_mlp_regressor(
                    residual_loaders, pinball_loss_fn, input_dim=INPUT_DIM, y_dim=1,
                    max_epochs=MAX_EPOCHS, lr=lr, l2reg=l2reg, return_best_model=True,
                    device=device, pos=True)
                if quantile_mlp_result['val_loss'] < best_pinball_loss:
                    best_pinball_loss = quantile_mlp_result['val_loss']
    ...
    for seed in SEEDS:
        ...
        for lr, l2reg in itertools.product(lrs, l2regs):
            model, result = train_mlp_regressor(
                loaders, loss_fn, input_dim=INPUT_DIM, y_dim=Y_DIM,
...
def main(args: argparse.Namespace) -> None:
    ...
    if shuffle:
        out_dir = f'out/storage_ptc{tag}_shuffle/'
    else:
        out_dir = f'out/storage_ptc{tag}/'
    ...
    if args.command == 'best_hp':
        ...
        func = functools.partial(
            ptc_box, shuffle=shuffle, device=device, out_dir=out_dir)
        ...
        func = functools.partial(
            ptc_ellipse, shuffle=shuffle, device=device, out_dir=out_dir)
        kwargs_list = [dict(alpha=alpha) for alpha in args.alpha]
        run_parallel(func, kwargs_list, workers=args.multiprocess)

    elif args.command == 'ptc_ellipse_johnstone':
        func = functools.partial(
            ptc_ellipse_johnstone, shuffle=shuffle, device=device, out_dir=out_dir)
        kwargs_list = [dict(alpha=alpha) for alpha in args.alpha]
        run_parallel(func, kwargs_list, workers=args.multiprocess)
...
if __name__ == '__main__':
    ...
    args = parse_args(commands, lr=False, l2reg=False)
    main(args)
